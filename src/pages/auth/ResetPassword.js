    ...
    return (
        <form onSubmit={handleSubmit} noValidate className={`${style['reset']} flex-col-center white-bg radius-5px shadow-2px`}>
            ...
            <div className={`${language === 'ar' ? style['arabic-reset--slot'] : style['reset--slot']} full-width flex-col-left-start margin-4px-V`}>
                ...
                <i className={`${language === 'ar' ? style['arabic-reset--slot--icon'] : style['reset--slot--icon']} orange size-20px bi bi-key`}></i>
                <i className={`${language === 'ar' ? style['arabic-reset--slot--icon--left'] : style['reset--slot--icon--right']} bi bi-eye${!passwordShown ? "-slash" : ""} size-20px pointer orange`} onClick={() => setPasswordShown(!passwordShown)} />
                <input
            ...
            <div className={`${language === 'ar' ? style['arabic-reset--slot'] : style['reset--slot']} full-width flex-col-left-start margin-4px-V`}>
                ...
                <i className={`${language === 'ar' ? style['arabic-reset--slot--icon'] : style['reset--slot--icon']} orange size-20px bi bi-key`}></i>
                <i className={`${language === 'ar' ? style['arabic-reset--slot--icon--left'] : style['reset--slot--icon--right']} bi bi-eye${!confirmShown ? "-slash" : ""} size-20px pointer orange`} onClick={() => setConfirmShown(!confirmShown)} />
                <input
            ...
            <button type="submit" className={`${style['reset--btn']} full-width orange-bg white inter pointer radius-10px shadow-2px`}>{translations.resetPassword}</button>
            <div className="full-width flex-row-left-start">
                <NavLink to={'/auth/forget-password'} style={{ textDecoration: 'none' }} className="space-none inter orange size-14px margin-12px-H pointer">{translations.back}</NavLink>
            </div>
        </form>
    );
};
