  ...
  return (
    <form onSubmit={handleForgetPassword} noValidate className={`${style['forget']} flex-col-center white-bg radius-5px shadow-2px`}>
      ...
      <div className="full-width flex-row-left-start">
        <NavLink to={'/store-panel/login'} style={{ textDecoration: 'none' }} className={`space-none inter ${mode === 'dark-mode' ? 'gray' : 'mint-green'} size-14px margin-12px-H pointer`}>{translations.login}</NavLink>
      </div>
    </form>
  );
};
